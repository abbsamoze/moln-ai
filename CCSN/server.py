...
#                     "id": str(df["id"][i]),
#                     "title": df["title"][i],
#                     "author": df["author"][i] if not type(df["author"][i]) == float else "N/A",
#                     "image": df["image"][i],
#                     "keywords": df["keywords"][i]
#                     }
#     recommendations = get_recommendations(int(id), num_recommend=13)
#     for i in recommendations["author"]:
#         if type(recommendations["author"][i]) == float:
#             recommendations["author"][i] = "N/A"
#     return jsonify(
#         {
#             "bookData": bookData,
...
def predictor(img):
    ...
    #return jsonify(predictedresults)
    returnData = {
        "Clas": str(clas),
        "Shape": name,
        "altitude": altitude,
    ...
    #print(jsonData) #prints the final json object
    return jsonData
...
def sendPredict(): 
    return predictor('data\Predict\Cumulus\Cu-N158.jpg') #set filepath to img for prediction
